    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            if (RunningPlatform() == Platform.Windows)
            {
                ...
                if (!(DefaultDir.LastIndexOf(slash) == DefaultDir.Length - 1))
                {
            ...
            tabControl1.TabPages.Remove(tabPage10); //don't show web connection page as it's a prototype 
            ...
            if (!industrial)
            {
                this.Text = "UltraMarker                   " + theVersion + "  " + platformStr + " for .NET4              GNU GPL v3 project managed by N. Palmer " + DateTime.Today.Year + "                    (F1 for help)";
            }
            else
            {
                this.Text = "UltraMarker Security Compliance Assessor   " + theVersion + "  " + platformStr + " for .NET4            GNU GPL v3 project managed by N. Palmer " + DateTime.Today.Year + "                    (F1 for help)";
            }
        ...
        {   //save student mark
            ...
            string str = marksDirectory + slash + assess.Code;
            ...
            try
            {
                ...
                {   //check if you've lost connection to online resource nad if so save locally instead temporarily
                    ...
                    if (ret == DialogResult.Yes)
                    {
                        ...
                        {
                            marksDirectory = ConfigDir + "Temp" + slash + assess.Code;
                        }
                        else
                        {
                            marksDirectory = ConfigDir + unitCodeTextBox.Text + slash + assess.Code;
                        }
                        str = marksDirectory;
                    }
                    else
            ...
            if (dialogResult3 == DialogResult.Yes)
            {
                if (StudentcomboBox.Text.Trim().Length > 0)
                {
                    ...
                    if (Sitting != "Main")
                    {
                        saveFileDialog3.FileName = StudentcomboBox.Text + addstr + "-" + Sitting;
                    }
                    else
                    {
        ...
        private void Load_Marked(bool LoadFF)
        {
            ...
            openFileDialog3.InitialDirectory = marksDirectory + slash + assess.Code;
        ...
        {
            string NPath = marksDirectory + slash + assess.Code + slash + Name;
        ...
        {   //save units
            if (UnitTitletextBox.Text != null && UnitTitletextBox.Text.Length > 0)
            {
                ...
                else
                {
                    ...
                    marksDirectory = UnitFilePath + slash + "Marks";
        ...
        private bool editDefaultDir(string message)
        {
            ...
            if (reply == DialogResult.Yes)
            {
                if (str.LastIndexOf(slash) == str.Length - 1) //is last charcater a slash?
                { sl = ""; }
                else { sl = slash; }
                DefaultDir = str + sl;
                GradeFile = "";
        ...
        { //starts with marked file: and ends with contains percent:
            ...
            if (File.Exists(filename))
            {
                ...
                try
                {
                                ...
                                try
                                {
                                    ...
                                    if (str.Contains("■"))
                                    {
                                        ...
                                        {                                           
                                            ...
                                            {
                                                Mstr = Mstr.Substring(Mstr.IndexOf("M") + 1);
                                                M = Convert.ToInt32(Mstr);
                                            }
                                            else
                                            {
                                                MessageBox.Show("Mark weight format error");
                                                M = 1;
                                            }
                                        ...
                                        {
                                            MessageBox.Show("Mark weight error");
                                            M = 1;
                                        }
                                    }
                                    int dot = str.IndexOf(".");
                                    ...
                                    if ((task > nodecount) && (first1))
                                    {
                                        MessageBox.Show("Task " + task + " may need sub-criteria - reverting task to zero");
                                        task = 0;
                                        first1 = false;
                                    }
                                    if (task > -1)  //criteria tasks must start from ZERO
                                    {
                    ...
                    //Now show grades in treeview:
                    try
                    {
                        for (int cnum = 0; cnum == cr; cnum++)
                        {
                            treeView2.SelectedNode = treeView2.Nodes[0].Nodes[cnum]; //select the node for this criteria
                            int ncount = treeView2.Nodes[0].Nodes[cnum].GetNodeCount(false);
                            //if (ncount > 0)
                            if (ncount > -1)
                            {
                                //sub-criteria
                                for (int n = 0; n < treeView2.Nodes[0].Nodes[cnum].GetNodeCount(false); n++)
                                {
                                    treeView2.SelectedNode = treeView2.Nodes[0].Nodes[cnum].Nodes[n];
                                    //overrideBox.Text = Marks[cnum, n, Session];
        ...
        private void startImport()
        {
            if (EditStudent && startMark && (importFileBox.Text.Trim() != ""))
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    importFromFileNew(importFileBox.Text);
                }
            }
            else
            {
        ...
        private TextBox EditSpecific(TextBox box)
        {
            ...
            box.Text = addcommentForm.Passvalue[1];
            if (addcommentForm.ComFile != CommentFile && addcommentForm.ComFile != null)
            {
                CommentFile = addcommentForm.ComFile;
            }
